...
public class PtrFrameLayout extends ViewGroup {
    ...
    public static final int PERFORM_REFRESH_COMPLETE_DELAYED = 500;
    ...
    public boolean dispatchTouchEvent(MotionEvent e) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mDisableWhenHorizontalMove && !mPreventForHorizontal && (Math.abs(e.getX() - mDownX) > mPagingTouchSlop && Math.abs(offsetX) > Math.abs(offsetY * mPtrIndicator.getResistance()))) {
    ...
    /**
     * add by lai 2015-11-18   不用返回结果则用原本的  {@link #refreshComplete()}
     ...
     */
    final public void refreshComplete(boolean success) {
        if (DEBUG) {
        ...
        if (mPtrUIHandlerHolder.hasHandler()) {
            ...
            mPtrUIHandlerHolder.onUIRefreshSuccess(this, success);
        }
        postDelayed(mPerformRefreshCompleteDelay, PERFORM_REFRESH_COMPLETE_DELAYED);
    }
    ...
    public void autoRefresh() {
        autoRefresh(false, mDurationToCloseHeader);
    }
