...
public class DialogUtilActivationCode {
	...
	public DialogUtilActivationCode(Context context, final CallBackConfirm2 callBackConfirm2){
		...
		dialog_default_address_confirm.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if(TextUtils.isEmpty(et_dialog_default_address_pay_pwd.getText().toString().trim())){
					ToastUtil.showMyLongToast("请输入激活码");
					return;
				}
				dialog.dismiss();
